 ...
 */
for ($i = 1; $i <= 29; $i++) {
    $textInput{$i} = new Quform_Element('text_input' . $i);
    $textInput{$i}->addFilter('trim');
    $textInput{$i}->addValidator('required');
    $form->addElement($textInput{$i});
}
